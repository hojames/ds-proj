...
zMS_KY <- zscores %>% filter(State== "Mississippi" | State == "Kentucky") # two states with declinine LE

save(zMS_KY, file="Z-scores_MS_KY")
...
```{r}
varKY <- varKY %>% mutate_all(funs(ifelse(is.na(.), 0, .)))
KY.pca <- prcomp(varKY,
                 center = TRUE,
                 scale. = TRUE) 
summary(KY.pca)
...
```{r}
loadings <- as.data.frame(KY.pca$rotation) #create a data frame of PCA loadings
melted <-  melt(KY.pca$rotation[,1:4])
